...
# Page config
st.set_page_config(page_title="GridSense", 
                   page_icon="🔌", 
                   layout="wide")
...
# ── SECTION 3 — Hourly & Monthly Patterns ──
st.markdown("---")
...
st.markdown("Enter meter readings manually and get instant prediction.")
...
if st.button("🔍 Predict", use_container_width=True):
    # Calculate deviation for input
    features_mlr  = ['Voltage', 'Global_intensity',
                      'Sub_metering_1', 'Sub_metering_2', 'Sub_metering_3']
    mlr2 = LinearRegression()
    mlr2.fit(df[features_mlr], df['Global_active_power'])
...
col_bill1.metric("Avg Power (last 3 months)", f"{last_3_avg:.3f} kW")
col_bill2.metric("Estimated Units",            f"{units:.0f} kWh")
col_bill3.metric("Predicted Bill",             f"₹{bill:,.0f}")
